...
public class UserService implements UserServicePort{
	...
	@Override
	public ResponseDTO getToken(String userName, String password) {
		...
		if(userName.isBlank() || userName.isEmpty() || password.isBlank() || password.isEmpty()) {
			return createResponseDTO("error", "username and password must bot be blank.", 400);
		}
	    ...
	    // Consuming rest API
	    	...
	    	try {
				...
				return createResponseDTO("error", "Invalid credentials", 400);
			}
	    	JSONObject obj = new JSONObject(response.getBody());
	    	if (obj.has("access_token")) {
	    		String access_token = obj.getString("access_token");
				return createResponseDTO("token", access_token, 200);
	    	}
	    	if (obj.has("error")) {
	    		String errorDesc = obj.getString("error_description");
	    		String error = obj.getString("error");
				return createResponseDTO(error, errorDesc, 400);
	    	}
		return createResponseDTO("error", "something went wrong! Please try again...", 400);
	}
	
	public ResponseDTO createResponseDTO(String name, String message, int statusCode) {
