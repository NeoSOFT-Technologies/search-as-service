...
public class KeycloakRolesManagement {
	...
	public void fetchKeycloakRolesAndMaintainCache() {
		...
		if(!KeycloakRolesIMC.KEYCLOAK_REALM_ROLES.containsKey(realmName)) {
			...
			KeycloakRolesIMC.KEYCLOAK_REALM_ROLES.put(
					realmName, 
					BasicUtil.parseJsonArrayToListOfMaps(jsonArrayRealmRoles.get()));
		} else {
			jsonArrayRealmRoles = Optional.ofNullable(new JSONArray(KeycloakRolesIMC.KEYCLOAK_REALM_ROLES.get(realmName)));
		}
		...
		if(!KeycloakRolesIMC.KEYCLOAK_CLIENT_ROLES.containsKey(saasClientTenantId1)) {
			...
			KeycloakRolesIMC.KEYCLOAK_CLIENT_ROLES.put(
					saasClientTenantId1, 
					BasicUtil.parseJsonArrayToListOfMaps(jsonArrayTenantRoles1.get()));
		} else {
			jsonArrayTenantRoles1 = Optional.ofNullable(new JSONArray(KeycloakRolesIMC.KEYCLOAK_CLIENT_ROLES.get(saasClientTenantId1)));
		}
		...
		if(!KeycloakRolesIMC.KEYCLOAK_CURRENT_USER_ROLES.containsKey(KeycloakRolesIMC.ACTIVE_USERNAME)) {
			String url = keycloakClientRoleUrl + "/" + realmName + "/users" + "/" + KeycloakRolesIMC.ACTIVE_USERNAME + "/role-mappings";
			KeycloakConfigAdapterResponse rolesInfoResponse = keycloakConfigAdapterPort.getRolesInfo(url);
			...
			if(tempObj instanceof JSONArray) {
				...
				KeycloakRolesIMC.KEYCLOAK_CURRENT_USER_ROLES.put(
						KeycloakRolesIMC.ACTIVE_USERNAME, 
						BasicUtil.parseJsonArrayToListOfMaps(jsonArrayUserRoles.get()));
			}

		} else {
			jsonArrayUserRoles = Optional.ofNullable(new JSONArray(KeycloakRolesIMC.KEYCLOAK_CURRENT_USER_ROLES.get(KeycloakRolesIMC.ACTIVE_USERNAME)));
		}
		
		// GET details for current user
		Optional<JSONArray> jsonArrayUserDetails;
		if(!KeycloakRolesIMC.KEYCLOAK_CURRENT_USER_DETAILS.containsKey(KeycloakRolesIMC.ACTIVE_USERNAME)) {
			String url = keycloakClientRoleUrl + "/" + realmName + "/users" + "/" + KeycloakRolesIMC.ACTIVE_USERNAME;
			...
			if(tempObj instanceof JSONArray) {
				...
				KeycloakRolesIMC.KEYCLOAK_CURRENT_USER_DETAILS.put(
						KeycloakRolesIMC.ACTIVE_USERNAME, 
						BasicUtil.parseJsonArrayToListOfMaps(jsonArrayUserDetails.get()));
			}

		} else {
			jsonArrayUserDetails = Optional.ofNullable(new JSONArray(KeycloakRolesIMC.KEYCLOAK_CURRENT_USER_DETAILS.get(KeycloakRolesIMC.ACTIVE_USERNAME)));
		}
		
		
		// GET details for given client
		Optional<JSONArray> jsonArrayClientDetails;
		if(!KeycloakRolesIMC.KEYCLOAK_CLIENT_DETAILS.containsKey(saasClientTenantId1)) {
			...
			if(tempObj instanceof JSONArray) {
				...
				KeycloakRolesIMC.KEYCLOAK_CLIENT_DETAILS.put(
						saasClientTenantId1, 
						BasicUtil.parseJsonArrayToListOfMaps(jsonArrayClientDetails.get()));
			}

		} else {
			jsonArrayClientDetails = Optional.ofNullable(new JSONArray(KeycloakRolesIMC.KEYCLOAK_CLIENT_DETAILS.get(saasClientTenantId1)));
		}
		
		
		// TESTING............
		Optional<JSONArray> jsonArrayCustomDetails;
		if(!KeycloakRolesIMC.KEYCLOAK_TEST_DETAILS.containsKey(saasClientTenantId1)) {
			String url = keycloakClientRoleUrl + "/" + realmName + "/clients" + "/" + "933b7340-7345-4529-ba62-6acbaabaafa9" + "/evaluate-scopes/generate-example-access-token";
			...
			if(tempObj instanceof JSONArray) {
				...
				KeycloakRolesIMC.KEYCLOAK_TEST_DETAILS.put(
						saasClientTenantId1, 
						BasicUtil.parseJsonArrayToListOfMaps(jsonArrayCustomDetails.get()));
			}

		} else {
			jsonArrayCustomDetails = Optional.ofNullable(new JSONArray(KeycloakRolesIMC.KEYCLOAK_TEST_DETAILS.get(saasClientTenantId1)));
		}
		
		
		log.debug("key_cl_roles >>> {}", KeycloakRolesIMC.KEYCLOAK_CLIENT_ROLES);
		log.debug("key_realm_roles >>> {}", KeycloakRolesIMC.KEYCLOAK_REALM_ROLES);
		log.debug("key_user_roles >>> {}", KeycloakRolesIMC.KEYCLOAK_CURRENT_USER_ROLES);
		log.debug("key_user_details >>> {}", KeycloakRolesIMC.KEYCLOAK_CURRENT_USER_DETAILS);
		log.debug("key_client_details >>> {}", KeycloakRolesIMC.KEYCLOAK_CLIENT_DETAILS);
