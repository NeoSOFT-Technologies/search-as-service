...
@RestController
@RequestMapping("${base-url.api-endpoint.versioned-rabbit-mq}")
public class VersionedRabbitMQResource {
	private final Logger log = LoggerFactory.getLogger(VersionedRabbitMQResource.class);
	
	private final RabbitMQSenderService rabbitMQSenderService;
	public VersionedRabbitMQResource(RabbitMQSenderService rabbitMQSenderService) {
	...
	public ApiResponseDTO sendData(@RequestBody String payload) {
		log.debug("Add_To_RabbitMQ");

		rabbitMQSenderService.Sender(payload);
		if (!payload.isEmpty()) {
			return new ApiResponseDTO(200, "Payload sent successfully");
		} else
