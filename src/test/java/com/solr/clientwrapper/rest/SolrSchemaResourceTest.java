...
class SolrSchemaResourceTest {
	...
	public void setMockitoSucccessResponseForService() {
		SolrSchemaDTO solrResponseDTO = new SolrSchemaDTO(tableName, name, attributes);
	...
	public void setMockitoBadResponseForService() {
		SolrSchemaDTO solrResponseDTO = new SolrSchemaDTO(tableName, Mockito.any(), attributes);
...
//
//		solrSchemaDTO = new SolrSchemaDTO(tableName, Mockito.any(), attributes);
//		restAMockMvc.perform(MockMvcRequestBuilders.delete(solrendpoint + "/delete/" + tableName + "/" + Mockito.any())
//				.contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(solrSchemaDTO)))
//				.andExpect(status().isOk());
	}
	...
	void testDeleteSchema() throws Exception {
		...
		SolrSchemaDTO solrSchemaDTO = new SolrSchemaDTO(tableName, Mockito.any(), attributes);
		restAMockMvc.perform(MockMvcRequestBuilders.delete(solrendpoint + "/delete/" + tableName + "/" + Mockito.any())
				.contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(solrSchemaDTO)))
				.andExpect(status().isBadRequest());
	...
	void testUpdateSchema() throws IOException, Exception {
		...
		// update Schema
		solrSchemaDTO = new SolrSchemaDTO(tableName, Mockito.any(), attributes);
		restAMockMvc.perform(MockMvcRequestBuilders.put(solrendpoint + "/update/" + tableName + "/" + Mockito.any())
	...
	void testGetSchema() throws Exception {
		...
		// GET CREATED SCHEMA
		restAMockMvc.perform(MockMvcRequestBuilders.get(solrendpoint + "/getSchema/" + tableName + "/" + Mockito.any())
