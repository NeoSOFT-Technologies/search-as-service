...
class TableDeletionTest {
	...
	void testTableDeleteInitialize() {
		...
		assertEquals(200,
				tableDeleteService.initializeTableDelete(1, "TestTable_10_101", new LoggersDTO()).getStatusCode());
	...
	void testTableDeletion() {
		...
		when(tableDeleteServicePort.checkDeletionofTable(Mockito.any())).thenReturn(1);
		try {
		...
		when(tableDeleteServicePort.checkTableDeletionStatus(1)).thenReturn(true);
